# Filter Kamera SUUCHAT — 12 Kategori (Metode TikTok Effect House)

Peta lengkap **12 kategori** filter: **fungsi → node utama Effect House → kode inti GLSL → status di SUUCHAT**.
SUUCHAT berjalan di **web (Canvas2D + WebGL + MediaPipe)**, jadi ekuivalen Effect House diimplementasikan dengan MediaPipe FaceLandmarker/Segmenter + Canvas + shader. Status: ✅ ada · 🟡 versi ringkas/aproksimasi · 🔜 roadmap.
...
- **SUUCHAT:** `lib/bilateral.ts` (shader bilateral) + mask wajah/badan (`faceFx.smoothInto`, `applyBodySkin`). Tint kulit ikut `tintStrength()` agar tak memucatkan.
...
- **SUUCHAT:** `faceFx.applyMakeup` (poligon landmark + blend) + preset `makeup` di `lib/filters.ts`
...
- **SUUCHAT:** `lib/meshWarp.ts` (Delaunay + warp affine per segitiga). Guard anti-"hancur": skip segitiga terbalik, clamp pergeseran ≤0.1·faceW, cap ≤0.22.
...
- **SUUCHAT:** `faceFx.applyBackground` (MediaPipe Selfie Segmentation). Mode: Blur · Hologram · Studio · **Bokeh/Pantai/Senja/Alam** (scene prosedural) · **📷 Foto custom** (galeri)
...
- **SUUCHAT:** pendekatan **CSS filter** (`sepia/contrast/saturate/hue-rotate`) di kategori `Retro` (Vintage, Film 90s, B&W). 🔜 LUT PNG (`.png` Hald CLUT) untuk grading sinematik.
...
- **SUUCHAT:** `LiveCamera` efek `matagede`, `balon`, `fusiinos1206` (liquify/magnify per landmark)
...
- **SUUCHAT:** `LiveCamera.stepParticles` + map `PARTICLE` (Glitter 🌟, Bunga 🌸, Salju ❄️, Hati 💖). Dirender di lapisan paling atas.
...
- **SUUCHAT:** kategori **Seni** — Kartun, Pop Art, Anime (aproksimasi via CSS filter + makeup/reshape, **bukan** style-transfer ML). 🔜 style-transfer asli (tfjs/WebGL CNN) butuh model & GPU lebih berat.
...
- **SUUCHAT:** 🔜 belum ada. Fondasi siap: FaceLandmarker sudah jalan; tinggal tambah logika kedip + state skor + objek jatuh.
...
- **SUUCHAT:** versi **2D vektor** `LiveCamera.drawEffect` (Anting 💎 di telinga, Kalung 📿 di bawah dagu). 🔜 model 3D PBR (mis. anting brand) via three.js.

---

## Pemetaan ke UI SUUCHAT (Live → ✨ Efek)
